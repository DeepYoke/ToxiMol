...
def process_molecule(
    ...
    Args:
        agent: DeepseekVLV2Agent instance
        molecule: Molecule data dictionary
        task_name: Name of the task
        task_prompt: Task prompt dictionary
        repair_prompt: Main repair prompt dictionary
        ...
        try:
            if i > 0 and i % 10 == 0:
                if torch.cuda.is_available():
                    ...
                    import gc
                    gc.collect()
                    logger.info(f"Cleared GPU memory after processing {i} molecules")
            ...
            logger.error(f"Error processing molecule {molecule['id']}: {e}")
            error_result = {
                "task": molecule["task"] if "task" in molecule else task_name,
                "molecule_id": molecule["id"],
                "original_smiles": molecule["smiles"],
