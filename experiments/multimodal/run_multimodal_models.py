#!/usr/bin/env python3
"""
Toxicity Repair Experiment Runner using various multimodal models via 智增增 API.

This script runs toxicity repair experiments on molecules using various multimodal models
(glm-4v-plus, qwen2.5-vl-72b-instruct, moonshot-v1-128k-vision-preview,
hunyuan-vision, deepseek-chat, yi-vision)
through 智增增 API service. It can process single tasks or run batch experiments.
"""
...
logger = logging.getLogger("multimodal_toxicity_repair")
...
# All available multimodal models
AVAILABLE_MODELS = [
    "glm-4v-plus",             # 支持图像输入的视觉模型
    "qwen2.5-vl-72b-instruct", # 支持图像输入的视觉-语言模型
    "moonshot-v1-128k-vision-preview", # 支持图像输入的视觉模型
    "hunyuan-vision",          # 支持图像输入的视觉模型 
    "deepseek-chat",           # 纯文本模型
    "yi-vision"                # 支持图像输入的视觉模型
]
...
def call_zhizengzeng_api(
    ...
    # 大多数模型支持temperature参数（根据各模型具体情况可能需要调整）
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run toxicity repair experiments using various multimodal models via 智增增 API")
    ...
    parser.add_argument(
        ...
        default="glm-4v-plus",
        help="Multimodal model to use (default: glm-4v-plus)"
    )
    ...
    if args.model not in VISION_MODELS:
        logger.warning(f"警告: 模型 {args.model} 不支持图像输入，请使用VISION_MODELS中的模型: {', '.join(VISION_MODELS)}")
