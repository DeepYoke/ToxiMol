#!/usr/bin/env python3
"""
Toxicity Repair Experiment Runner using Claude models via 智增增 API - No Image Ablation Version.

This script runs toxicity repair experiments on molecules using Claude models
through 智增增 API service WITHOUT providing molecule images. This is part of an ablation study
to assess the impact of visual information on the model's performance.
"""
...
logger = logging.getLogger("claude_toxicity_repair_no_image")

# Constants
BASE_DIR = Path(__file__).resolve().parent.parent.parent
DATA_DIR = BASE_DIR / "data" / "Experimental_dataset"
RESULTS_DIR = Path(__file__).resolve().parent / "results_no_image"  # 使用不同的结果目录
...
def create_repair_request(
    ...
    """
    Create the messages for the API request without image.
    ...
    # Create the system message from the repair prompt - same as original but without image reference
    system_content = (
    ...
    # Enhanced prompt to get better responses - same as original
    user_content = (
    ...
    messages = [
        ...
        {"role": "user", "content": user_content}  # 纯文本，不添加图像
    ]
...
def process_molecule(
    ...
    # 注意：不再需要图像路径，但我们仍然检查它是否存在，以保持与原始脚本的一致性
    image_path = DATA_DIR / task_name / "image" / f"{molecule_id}.png"
    if not os.path.exists(image_path):
        logger.warning(f"Image not found: {image_path} (but not used in this ablation experiment)")
    ...
    try:
        ...
        results["model"] = model
        results["no_image"] = True  # 标记这是无图像的消融实验
...
def run_task(
    ...
    logger.info(f"Running task {task_name} with {len(molecules)} molecules (no image ablation)")
    ...
    combined_results = {
        ...
        "results": all_results,
        "ablation_type": "no_image"  # 标记这是无图像的消融实验
    }
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run toxicity repair experiments using Claude models via 智增增 API (No Image Ablation)")
    ...
    logger.info(f"Starting no-image ablation experiment with model: {args.model} via 智增增 API")
    ...
    logger.info("No-image ablation experiment completed")
